...
class ProteinMoleculeDataset(Dataset):
    def __init__(self, reaction_data,  prot_obj):
    ...
    def __getitem__(self, idx):
        ...
        out = MultiGraphData(
                ## PROTEIN
                prot_node_aa=prot_node_esm, prot_node_evo=prot_node_evo,prot_node_pos=prot_node_pos, prot_seq=prot_seq,
                prot_edge_index=prot_edge_index, prot_edge_weight=prot_edge_weight,prot_num_nodes=prot_num_nodes,
                ## Y output,keys
                reg_y=reg_y, prot_key = prot_key)
        return out

class PairDataset(Dataset):
    def __init__(self, reaction_data, mol_obj, prot_obj,ecfp):
    ...
    def __getitem__(self, idx):
        ...
        #total_fea = mol["total_fea"].float()
        total_fea = torch.from_numpy(self.ecfp[mol_key]).float()
        #print(total_fea.size)
        ...
        out = MultiGraphData(
                ## MOLECULE Pan1103 total_fea
                mol_x=mol_x, mol_x_feat=mol_x_feat, total_fea=total_fea, mol_edge_index=mol_edge_index, mol_edge_attr=mol_edge_attr, mol_num_nodes= mol_num_nodes,
                ## PROTEIN
                prot_node_aa=prot_node_esm, prot_node_evo=prot_node_evo,prot_node_pos=prot_node_pos, prot_seq=prot_seq,
                prot_edge_index=prot_edge_index, prot_edge_weight=prot_edge_weight,prot_num_nodes=prot_num_nodes,
                ## Y output,keys
