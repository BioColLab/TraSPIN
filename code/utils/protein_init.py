...
def get_T5_model():
    ...
    model = T5EncoderModel.from_pretrained("/scratchdata1/users/a1242408/Dataset/protT5/protT5_checkpoint/").to(device)#/home/pantong/Code/ProtT5/
    model = model.to(dtype=torch.float16)
    model.half()
    model = model.eval() # set model to evaluation model
    tokenizer = T5Tokenizer.from_pretrained("Rostlab/prot_t5_xl_uniref50", do_lower_case=False )
    #tokenizer = T5Tokenizer.from_pretrained('/scratchdata1/users/a1242408/Dataset/protT5/', do_lower_case=False,force_download=True)#"/home/pantong/Code/ProtT5/"
...
def ESM_init(seq):
    ...
    result_dict= {
            'seq':seq,
            'seq_feat':None,#torch.from_numpy(seq_feat), #atom/resdue type + property
...
def protein_init(Sequence): #,organism
    ...
    for seq in tqdm(Sequence):
        ...
        result_dict[seq] = {
            'seq':seq_max,
            'seq_feat':None, #atom/resdue type + property
