...
class MoleculeGraphDataset():
    def __init__(self, atom_classes=None, halogen_detail=False, save_path=None):
        ...
        if atom_classes is None:
            metals = ([3, 4, 11, 12, 13] + list(range(19, 32)) + list(range(37, 51)) + list(range(55, 84)) + list(
                range(87, 104)))
        ...
        ## Bond feature
        self.edge_dict = {BondType.SINGLE: 1, BondType.DOUBLE: 2, BondType.TRIPLE: 3, BondType.AROMATIC: 4,
        ...
        for atom in mol.GetAtoms():
            ...
            onehot_label = one_of_k_encoding_unk(atom.GetSymbol(),
                                                 ['C', 'N', 'O', 'S', 'F', 'Si', 'P', 'Cl', 'Br', 'Mg', 'Na', 'Ca',
                                                  'Fe', 'As', 'Al',
                                                  'I', 'B', 'V', 'K', 'Tl', 'Yb', 'Sb', 'Sn', 'Ag', 'Pd', 'Co', 'Se',
                                                  'Ti', 'Zn', 'H',
                                                  'Li', 'Ge', 'Cu', 'Au', 'Ni', 'Cd', 'In', 'Mn', 'Zr', 'Cr', 'Pt',
                                                  'Hg', 'Pb',
                                                  'Unknown'])
...
def tree_decomposition(
    ...
    # Add singleton cliques in case there are more than 2 intersecting cliques. We further compute the "initial" clique graph.
    edges = {}
...
def smiles2graph(smile_str):
    mgd = MoleculeGraphDataset(halogen_detail=False)
    mol = Chem.MolFromSmiles(smile_str)
    # mol = get_mol(m_str)
    atom_feature, bond_feature = mgd.featurize(mol, 'atom_full_feature')  # atom property , edge type
    atom_idx, _ = mgd.featurize(mol, 'atom_type')  # atom type
...
def ligand_init(smiles_list):
    ...
    for smiles in tqdm(smiles_list):
        ...
        ligand_dict[smiles]["total_fea"] = torch.tensor(smiles_to_vec([smiles]))  # None#
