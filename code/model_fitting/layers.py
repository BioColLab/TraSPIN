...
from torch_geometric.utils import softmax, degree, subgraph, to_scipy_sparse_matrix, segregate_self_loops, \
...
class GCNNet(torch.nn.Module):
    def __init__(self, hidden_channels):
        ...
        list_Conv_layers = [GCNConv(hidden_channels, hidden_channels) for _ in range(3)]
...
class GCNCluster(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        # Input Layer Norm
        if self.in_norm:
            y = self.in_ln(y)  # (429,200)

        for idx in range(self.hidden_layers):
            y = self.Conv_layers[idx](y, edge_index)
            y = F.relu(y)  # (429,400)
        y = self.Conv_layers[-1](y, edge_index)  # (429,10)
...
class PositiveLinear(nn.Module):
    ...
    def forward(self, x):
        # Apply ReLU to ensure weights are positive
        positive_weights = torch.relu(self.linear.weight)
        return nn.functional.linear(x, positive_weights, self.linear.bias)
...
class PosLinear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True, init_value=0.2,
        ...
        weight = nn.init.uniform_(torch.empty((out_features, in_features), **factory_kwargs), a=lower_bound,
                                  b=upper_bound)
...
class Drug_PNAConv(nn.Module):
    def __init__(self, mol_deg, hidden_channels, edge_channels, pre_layers=2, post_layers=2,
                 aggregators=['sum', 'mean', 'min', 'max', 'std'],
                 scalers=['identity', 'amplification', 'attenuation'],
                 num_towers=4, dropout=0.1):
        super(Drug_PNAConv, self).__init__()

        self.bond_encoder = torch.nn.Embedding(5, hidden_channels)
        self.atom_conv = PNAConv(in_channels=hidden_channels, out_channels=hidden_channels,
                                 edge_dim=edge_channels, aggregators=aggregators, scalers=scalers, deg=mol_deg,
...
class DrugProteinConv(MessagePassing):
    ...
    def __init__(
        ...
        self.residue_mlp1 = nn.Linear(residue_channels, residue_channels)#MLP([residue_channels, residue_channels * 2, residue_channels], out_norm=True)
        ## temperature
        self.t = t
        # self.logit_scale = nn.Parameter(torch.ones([])) # * np.log(1 / 0.07))
    ...
    def forward(self, drug_x, residue_x, edge_index: Adj):
        ...
        H, aC = self.heads, self.atom_out_channels  # 5, 40
        residue_hx = residue_x  # self.residue_in_norm(residue_x)  # (B,3,200)
        query = self.lin_query(drug_x).view(-1, H, aC)  # (1,5,40)
        key = self.lin_key(residue_hx).view(-1, H, aC)  # (3,5,40)
        value = self.lin_value(residue_hx).view(-1, H, aC)  # (3,5,40)

        # Protein Residue -> Drug Atom
        drug_out = self.propagate(edge_index, query=query, key=key, value=value, edge_attr=None, size=None)  # (2,5,40)
        alpha = self._alpha  # (3,5)
        #print("alpha", alpha.size)
        self._alpha = None
        drug_out = drug_out.view(-1, H * aC)  # (1,200)
        # pan0415
        # drug_out = self.drug_out_norm(drug_out)
        drug_out = drug_x + F.relu(self.clique_mlp(drug_out))  # (1,200)

        # Drug Atom -> Protein Residue
        H, rC = self.heads, self.residue_out_channels
        drug_hx = drug_x  # self.drug_in_norm(drug_x)  # (1,200)# normalization
        residue_value = self.lin_atom_value(drug_hx).view(-1, H, rC)[edge_index[1]]  # (3,5,40)
        residue_out = residue_value * alpha.view(-1, H, 1)  # (3,5,40)
        residue_out = residue_out.view(-1, H * rC)  # (3,200)
        # pan0716
        residue_out = self.residue_out_norm(residue_out)
        residue_x = residue_x + F.relu(self.residue_mlp1(residue_out))#torch.cat([residue_out, residue_x], dim=-1)  # (5,400)
        #residue_out = self.residue_mlp(residue_out)  # (5,200)
        return drug_out, residue_x, (edge_index, alpha)

        # Protein Residue -> Drug Atom
        """H, aC = self.heads, self.atom_out_channels #5, 40
        residue_hx = self.residue_in_norm(residue_x) #(5,200)## normalization
        query = self.lin_query(drug_x).view(-1, H, aC) #(1,5,40)
        key = self.lin_key(residue_hx).view(-1, H, aC)#(5,5,40)
        value = self.lin_value(residue_hx).view(-1, H, aC)#(5,5,40)

        # propagate_type: (query: Tensor, key:Tensor, value: Tensor, edge_attr: OptTensor) # noqa
        drug_out = self.propagate(edge_index, query=query, key=key, value=value,edge_attr=None, size=None) #(1,5,40)
        alpha = self._alpha #(5,5)
        self._alpha = None

        drug_out = drug_out.view(-1, H * aC) #(1,200)
        drug_out = self.drug_out_norm(drug_out)
        #print("1111",clique_batch)
        clique_out = torch.cat([clique_x, drug_out[clique_batch]], dim=-1) #(24,400)
        clique_out = self.clique_mlp(clique_out)#(24,200)

        # Drug Atom -> Protein Residue 
        H, rC = self.heads, self.residue_out_channels
        drug_hx = self.drug_in_norm(drug_x) #(1,200)# normalization
        residue_value = self.lin_atom_value(drug_hx).view(-1, H, rC)[edge_index[1]]#(5,5,40)
        residue_out = residue_value * alpha.view(-1, H, 1)  #(5,5,40)
        residue_out = residue_out.view(-1, H * rC) #(5,200)
        residue_out = self.residue_out_norm(residue_out)
        residue_out = torch.cat([residue_out, residue_x], dim=-1) #(5,400)
        residue_out = self.residue_mlp(residue_out) #(5,200)
...
def dropout_edge(edge_index: Tensor, p: float = 0.5, force_undirected: bool = False, training: bool = True) -> Tuple[
    Tensor, Tensor]:
    ...
    Args:
        ...
        force_undirected (bool, optional): If set to :obj:`True`, will either
            drop or keep both edges of an undirected edge.(default: :obj:`False`)
        training (bool, optional): If set to :obj:`False`, this operation is a
            no-op. (default: :obj:`True`)
    ...
    Examples:
        ...
        >>> edge_index, edge_id = dropout_edge(edge_index,force_undirected=True)
        >>> edge_index
        tensor([[0, 1, 2, 1, 2, 3],
                [1, 2, 3, 0, 1, 2]])
